...
export function mongoDBAtlas(options?: IMongoDBAtlasOptions) {
  return async function (c: Context, next: Next) {
    ...
    if (!realmAppId)
      throw new Error(
        "options.realmAppId and env.REALM_APP_ID is not configured"
      );

    const realmApiKey = opts.realmApiKey || c.env.REALM_API_KEY;
    if (!realmApiKey)
      throw new Error(
        "options.realmApiKey and env.REALM_API_KEY is not configured"
      );
    ...
    function getDb(name: string): MongoDBDatabase {
      if (!dbCache[name]) dbCache[name] = client.db(name);
      return dbCache[name];
    }

    const collectionCache: Record<
      string,
      MongoDB.MongoDBCollection<Document>
    > = {};
    function getCollection<T extends Document>(
      ...
      if (!dbName)
        throw new Error(
          "defaultDb is not configured, you need to provide dbName explicitly"
        );
